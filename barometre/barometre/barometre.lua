...
barometr_v.0.1.lua par olgmen le 07.12.2010
script affichant les prévisions météorologiques avec http://weather.noaa.gov
Utiliser les scripts :
BARGRAPH WIDGET v2.0 par wlourf (12.07.2010)
écriture en cercle par mrpeachy (2010)
]]

require 'cairo'

-- [[ tous les paramètres sont entrés ici ]] --
...
{					-- thermomètre
name = "temperature",
arg = conky_parse('${execi 300 sed -n 3p /home/rafa/.conky/conky-draw/barometre/conditions.txt}'),
max = 80,
radius = 70,
thickness = 9,
sectors = 6,
...
{					-- température
text = "TEMPERATURE " .. conky_parse('${execi 300 sed -n 3p /home/rafa/.conky/conky-draw/barometre/conditions.txt}') .. "C",
font_name = "Droid Sans",		-- nom de la police
font_size = 8,				-- taille de la police
bold = true,
...
{					-- pressure en mb
text = "PRESSURE " .. conky_parse('${exec sed -n 1p $HOME/.conky/conky-draw/barometre/reptravail/pre}') .. "mb",
...
{					-- la vitesse du wind
text = "WIND " .. conky_parse('${execi 600 sed -n 6p $HOME/.conky/conky-draw/barometre/reptravail/curr_cond}') ,
font_name = "MonogramsToolbox",
font_size = 6,
bold = true,
start_angle = 63,
...
{					-- 	humidité
text = "HUMIDITY " .. conky_parse('${execi 600 sed -n 7p $HOME/.conky/conky-draw/barometre/reptravail/curr_cond}') .. "",
font_name = "MonogramsToolbox",
font_size = 6,
bold = true,
inside = true,
...
{					-- direction du wind
text = "DIR " .. conky_parse('${execi 600 sed -n 5p $HOME/.conky/conky-draw/barometre/reptravail/curr_cond}'),
font_name = "MonogramsToolbox",
font_size = 6,
bold = true,
start_angle = 240,
...
{
name = os.getenv ('HOME')..'/.conky/conky-draw/barometre/wind_rose.png',	-- chemin de l'image , pour modifier taille modifier le taille de l'image dans son dossier
},
...
arg = conky_parse('${texeci 600 sed -n 5p $HOME/.conky/conky-draw/barometre/reptravail/curr_cond}'),
max = 360,
...
{					-- pressure
name = "pressure",
arg = conky_parse('${execi 600 sed -n 1p $HOME/.conky/conky-draw/barometre/reptravail/pre}'),
max = 80,
start_angle = 242,
end_angle = 480,
...
-- paramètres par défaut
		...
		if t.x 			== nil then t.x = conky_window.width/12.6 end  -- originale  2 milieu conky
		if t.y			== nil then t.y = conky_window.height/2.25 + 14 end      -- = haut
...
-- vérification des paramètres d'entrée et réglage des paramètres par défaut
	...
	if t.x		== nil then t.x = conky_window.width/12.6 end --texte origiale = 2 milieu conky
	if t.y		== nil then t.y = conky_window.height/2.25 + 14 end  -- = haut
	if t.radius	== nil then t.radius = (conky_window.width/2)*0.9 end
...
-- paramètres par défaut
		...
		if t.name		== nil then t.name= "" end
		if t.x 			== nil then t.x = conky_window.width/12.6 end  --rose devant original 2 milieu conky
		if t.y			== nil then t.y = conky_window.height/2.25 + 14 end -- = haut
		if t.height		== nil then t.height = conky_window.width end
		if t.width		== nil then t.width = conky_window.width end
...
-- paramètres par défaut
	...
	if t.x 			== nil then t.x = conky_window.width/12.6 end  -- aiguille originale = 2 milieu conky
	if t.y			== nil then t.y = conky_window.height/2.30 + 14 end  -- = haut
	...
	if t.name == "wind" then
		...
		if t.arg	== "SSO"	then t.arg = 202.5 end
